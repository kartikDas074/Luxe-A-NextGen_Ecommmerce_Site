...
import { addOrder } from "@/lib/server/order";
...
export default function ProductDetailClient({
  ...
  const initialImage = activeImages[0] || "";
  ...
  const currentImage = activeImages.includes(mainImage)
    ? mainImage
    : initialImage;
  ...
  const handleSimulatedCheckout = async (e: React.FormEvent) => {
    ...
    const orderData = {
      ...
      brand: product.brand,
      featuredImage: activeImages[0] || "",
      selectedColor: selectedColor,
      quantity: quantity,
      deliveryRegion: deliveryRegion,
      estimatedDelivery: product.estimatedDelivery || "2-4 Business Days",
      unitPrice: currentProductUnitPrice,
      subtotal: itemsSubtotalAmount,
      shippingCharge: standardShippingCharge,
      grandTotal: estimatedTotalInvoice,
      status: "pending",
    };
    ...
    try {
      ...
      if (result.success)
        toast.success(
          `Order Placed Successfully! Grand Total: ৳${estimatedTotalInvoice}`,
        );
      else toast.error("Something went wrong,plz try to order later again");
    } catch (error) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          <div className="lg:col-span-7 space-y-12">
            <div className="grid grid-cols-12 gap-4 items-start">
              ...
              <div
                className={`bg-[#F9FAFB] rounded-2xl border border-gray-100 flex items-center justify-center relative overflow-hidden group ${
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {Array.isArray(product.specifications) &&
                  product.specifications.map((spec: any, idx: number) => (
                    <div
                      key={idx}
                      className="bg-white border border-gray-100 rounded-xl p-4 text-center shadow-xs space-y-1"
                    >
                ...
                {!product.specifications?.length && (
                  <>
                    <div className="bg-white border border-gray-100 rounded-xl p-4 text-center shadow-xs">
                      ...
                      <p className="text-xs font-black">4500mAh</p>
                    </div>
                    <div className="bg-white border border-gray-100 rounded-xl p-4 text-center shadow-xs">
                      <p className="text-[9px] font-bold text-gray-400 uppercase">
                        NOISE CANCEL
                      </p>
                      <p className="text-xs font-black">40dB ANC</p>
                    </div>
                    <div className="bg-white border border-gray-100 rounded-xl p-4 text-center shadow-xs">
                      <p className="text-[9px] font-bold text-gray-400 uppercase">
                        WEIGHT
                      </p>
                      <p className="text-xs font-black">260g</p>
                    </div>
                    <div className="bg-white border border-gray-100 rounded-xl p-4 text-center shadow-xs">
                      <p className="text-[9px] font-bold text-gray-400 uppercase">
                        CONNECTIVITY
                      </p>
                      <p className="text-xs font-black">v5.3 LE</p>
                    </div>
                  </>
                )}
              </div>
            ...
            <div className="space-y-4 pt-2">
              ...
              <p className="text-xs font-medium text-gray-500 leading-relaxed text-left whitespace-pre-line">
                {product.fullDescription ||
                  "The luxury configuration details and premium design aesthetics provide an unparalleled audio setup structure built purposefully for pure lifestyle immersion."}
              </p>
            </div>
          </div>
            ...
            <div className="space-y-4 text-left">
              <div className="flex items-center gap-2 text-xs text-gray-400 font-semibold">
                <span className="text-[#3B82F6] font-bold hover:underline cursor-pointer">
                  {product.brand || "Luxe Premium"}
                </span>
                <span>•</span>
                <div className="flex items-center text-amber-400 gap-0.5">
                  <Star className="w-3 h-3 fill-current" />{" "}
                  <span className="text-gray-900 text-[11px] font-bold ml-0.5">
                    4.8 (1,242 reviews)
                  </span>
                </div>
              </div>
                ...
                {discountAmount > 0 && (
                  <>
                    ...
                    <span className="text-[10px] font-black text-rose-500 bg-rose-50 border border-rose-100 px-1.5 py-0.5 rounded-sm">
                      {Math.round(
                        ((baseRegularPrice - discountAmount) /
                          baseRegularPrice) *
                          100,
                      )}
                      % OFF
              ...
              <p className="text-xs text-gray-500 leading-relaxed font-medium bg-gray-50 p-4 rounded-xl border border-gray-100">
                {product.shortDescription ||
                  "Experience uncompromising audio quality. Combines studio-grade sound with all-day comfort and industry-leading performance mechanics."}
              </p>
                ...
                <div className="flex items-center gap-2.5 bg-white border border-gray-100 p-3 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-gray-900 font-bold text-[10px]">
                      {product.warranty || "6 Months Warranty"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2.5 bg-white border border-gray-100 p-3 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-gray-900 font-bold text-[10px]">
                      {product.estimatedDelivery || "2-4 Business Days"}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-xl/5 space-y-5 relative">
              <div className="flex items-center justify-between border-b border-gray-50 pb-3">
                ...
                <span className="text-[9px] font-mono text-gray-400 bg-gray-50 border px-1.5 py-0.5 rounded">
                  Stock Matrix: {product.stock || 0} left
                </span>
              </div>
                ...
                <div>
                  ...
                  >
                    <option value="inside-dhaka">
                      🚚 Local Hub (Inside Dhaka Base Rate)
                    </option>
                    <option value="outside-dhaka">
                      🌐 Global Outpost (Outside Dhaka Matrix +৳60)
                    </option>
                  </select>
                </div>

                <div className="flex items-center justify-between gap-4 pt-1">
                  <span className="text-[9px] font-black uppercase text-gray-400 tracking-wider text-left">
                    Quantity
                  </span>
                ...
                <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 space-y-2 text-xs font-semibold text-gray-500">
                  ...
                  <div className="flex justify-between items-center border-t border-dashed border-gray-200 pt-2 mt-1 text-gray-900">
                    <span className="text-[10px] font-black uppercase tracking-wide">
                      Estimated Total Invoice
                    </span>
                    <span className="text-sm font-black font-mono">
                      ৳{estimatedTotalInvoice}
                    </span>
                  </div>
                </div>
